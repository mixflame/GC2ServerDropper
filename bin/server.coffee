...
ping_nexus = ->
  if is_private == false
    ...
    req = http.get('nexusnet.herokuapp.com', encodeURIComponent("/online?name=#{server_name}&host=#{host}&port=#{port}"), (res) ->
      log "Nexus Pinged."
    )
...
build_chat_log = ->
  ...
  out
clean_handles = ->
  for k, v of handle_keys when (handle_last_pinged[k] < (new Date().getTime() - 15000))
